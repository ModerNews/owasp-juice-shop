...
} from "./routes/orderHistory";
import {
  continueCode,
  continueCodeFindIt,
  continueCodeFixIt,
} from "./routes/continueCode";
import {
  serveChallengesWithCodeSnippet,
  serveCodeSnippet,
  checkVulnLines,
} from "./routes/vulnCodeSnippet";
import {
  ensureFileIsPassed,
  handleZipFileUpload,
  checkUploadSize,
...
restoreOverwrittenFilesWithOriginals()
  .then(() => {
    /* Locals */
    ...
    app.use(
      ...
      securityTxt({
        ...
        "Preferred-Languages": [
          ...new Set(
            locales.map((locale: { key: string }) => locale.key.substr(0, 2)),
          ),
        ].join(", "),
        hiring: config.get("application.securityTxt.hiring"),
        csaf:
          config.get<string>("server.baseUrl") +
          config.get<string>("application.securityTxt.csaf"),
      ...
      res.end = function () {
        if (arguments.length) {
          ...
          arguments[0] = arguments[0].replace(
            /a href="([^"]+?)"/gi,
            function (matchString: string, matchedUrl: string) {
    ...
    app.use(function jsonParser(
      ...
      if (req.headers["content-type"]?.includes("application/json")) {
        ...
        if (req.body !== Object(req.body)) {
          // Expensive workaround for 500 errors during Frisby test run (see #640)
      ...
      (req: Request, res: Response, next: NextFunction) => {
        if (
          ...
          if (req.body.email.length !== 0 && req.body.password.length !== 0) {
            ...
            res
              .status(400)
              .send(res.__("Invalid email/password cannot be empty"));
          }
        }
        next();
      },
    ...
    for (const { name, exclude, model } of autoModels) {
      ...
      if (name === "User") {
        ...
        resource.create.send.before(
          (
            req: Request,
            res: Response,
            context: { instance: { id: any }; continue: any },
          ) => {
            // vuln-code-snippet vuln-line registerAdminChallenge
            WalletModel.create({ UserId: context.instance.id }).catch(
              (err: unknown) => {
                console.log(err);
              },
            );
      ...
      if (name === "Challenge") {
        resource.list.fetch.after(
          (
            req: Request,
            res: Response,
            context: { instance: string | any[]; continue: any },
          ) => {
            for (let i = 0; i < context.instance.length; i++) {
              let description = context.instance[i].description;
              if (
                utils.contains(description, "<em>(This challenge is <strong>")
              ) {
                const warning = description.substring(
                  description.indexOf(" <em>(This challenge is <strong>"),
                );
                ...
                context.instance[i].description =
                  req.__(description) + req.__(warning);
              } else {
      ...
      if (name === "SecurityQuestion") {
        resource.list.fetch.after(
          (
            ...
            for (let i = 0; i < context.instance.length; i++) {
              context.instance[i].question = req.__(
                context.instance[i].question,
              );
            }
            return context.continue;
          },
        ...
        resource.read.send.before(
          (
            req: Request,
            res: Response,
            context: { instance: { question: string }; continue: any },
          ) => {
      ...
      if (name === "Product") {
        resource.list.fetch.after(
          (
            req: Request,
            res: Response,
            context: { instance: any[]; continue: any },
          ) => {
            for (let i = 0; i < context.instance.length; i++) {
              context.instance[i].name = req.__(context.instance[i].name);
              context.instance[i].description = req.__(
                context.instance[i].description,
              );
            }
            return context.continue;
      ...
      resource.all.send.before(
        (
          req: Request,
          res: Response,
          context: { instance: { status: string; data: any }; continue: any },
        ) => {
          context.instance = {
            status: "success",
...
const uploadToDisk = multer({
  storage: multer.diskStorage({
    destination: (req: Request, file: any, cb: any) => {
      ...
      cb(
        error,
        path.resolve("frontend/dist/frontend/assets/public/images/uploads/"),
      );
    },
...
while (
  !expectedModels.every((model) =>
    Object.keys(sequelize.models).includes(model),
  )
) {
  logger.info(
    `Entity models ${colors.bold(Object.keys(sequelize.models).length.toString())} of ${colors.bold(expectedModels.length.toString())} are initialized (${colors.yellow("WAITING")})`,
  );
}
logger.info(
  `Entity models ${colors.bold(Object.keys(sequelize.models).length.toString())} of ${colors.bold(expectedModels.length.toString())} are initialized (${colors.green("OK")})`,
);
...
export async function start(readyCallback?: () => void) {
  ...
  server.listen(port, () => {
    ...
    if (process.env.BASE_PATH !== "") {
      logger.info(
        colors.cyan(
          `Server using proxy base path ${colors.bold(`${process.env.BASE_PATH}`)} for redirects`,
        ),
      );
    }
